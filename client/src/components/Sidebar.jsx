...
const Sidebar = ({ handleLogout }) => {
  ...
  return (
    <div className="fixed top-0 z-20">
      <div
        className={`p-2 text-xl font-bold bg-primary rounded-md cursor-pointer mx-1 my-2 lg:hidden hover:bg-dark ${
          minimize ? "hidden" : "inline-block"
        }`}
      ...
      <div
        className={`bg-primary h-screen lg:w-52 w-34 fixed flex flex-col p-4 lg:block ${
          minimize ? "top-0 left-0 " : "top-0 -left-48 lg:left-0"
        } shadow-lg shadow-black transition-all duration-300 ease-in-out`}
      >
        <div className="text-3xl font-bold px-4 pt-6 pb-12 flex items-center gap-10 ">
          <p>IMS</p>
          <GrFormClose
            onClick={() => setMinimize(false)}
            className="lg:hidden hover:bg-dark rounded-md cursor-pointer"
          />
        </div>
